...
# %%
start_prop = re.compile(
    "===============================================================================\n(.*)\n(.*)", flags=re.MULTILINE)
propagations = re.compile("propagations: (.*)")
decisions = re.compile("decisions: (.*)")
total_cycle = re.compile("total_cycle: (.*)")
...
for cnf in cnfs:
    ...
    print("{} {} {} {}".format(start_prop.findall(txt)[-1],
                          propagations.findall(txt)[-1],
                          decisions.findall(txt)[-1],
                          total_cycle.findall(txt)[-1]))
